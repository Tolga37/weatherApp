...
import { Dimensions, NativeSyntheticEvent, Platform, StyleSheet } from 'react-native';
import {
    WebViewError,
    WebViewMessageEvent,
} from 'react-native-webview/lib/WebViewTypes';
import LoadingIndicator from './LoadingIndicator';
import { MapLayerType } from '../../../lib/enums/mapEnums';
...
export const LeafletView: React.FC<LeafletViewProps> = ({
    ...
    const DEFAULT_MAP_LAYERS = [
        {
            ...
            url: 'https://bms.basarsoft.com.tr/service/api/v1/map/Default?accId=aK3vB6cRokqMXzjIOiz_xA&appCode=9Nk1yAnYOEmrb56I-4JN7A&x={x}&y={y}&z={z}'
        },
    ];
        ...
        (event: WebViewMessageEvent) => {
            ...
            if (message.event === WebViewLeafletEvents.ON_MOVE_END) {
                ...
                if (message.event === WebViewLeafletEvents.ON_MAP_TOUCHED) {
                    logMessage(
                        `TOUCH: ${JSON.stringify(message.payload?.mapCenterPosition)}`
                    );
                }
            }
    ...
    useEffect(() => {
        Geolocation.getCurrentPosition(
            ...
            (position) => {
                ...
                //istanbul setledim şuanlık
                setLat("41.008240");
                setLng("28.978359");
                //setLat(currentLongitude);
                //setLng(currentLatitude);
                //Setting Longitude state
                console.log("LAAAAT" + currentLatitude)
                console.log("longg" + currentLongitude)
            },
            (error) => {
    ...
    LeafletView.defaultProps = {
        ...
        mapCenterPosition: { lat: lat, lng: lng },
        zoom: DEFAULT_ZOOM,
        doDebug: __DEV__,
    };
    return (
